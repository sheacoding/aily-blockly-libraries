...
- **Version**: 0.2.6
- **Runtime**: ESP32/ESP32-S3, WiFi, bundled ArduinoJson/WebSockets sources, legacy ESP-IDF I2S; optional ST7789/TFT_eSPI + LVGL screen with direct FT6336U-compatible I2C touch reads
...
6. **Screen dependencies are optional**: audio-only projects use the bundled ArduinoJson/WebSockets sources and do not inject LVGL, TFT_eSPI, FT6336U, or screen build macros. Any screen block enables `HUOSHAN_AI_SCREEN_ENABLED=1`, `LV_USE_TFT_ESPI=1`, ST7789/TFT_eSPI macros, and the screen runtime.
7. **Screen font**: the screen runtime includes the bundled `HuoshanAI_CN_15.inc` Source Han CJK LVGL font through `huoshan_ai_screen_font.h`, so audio-only builds do not compile the screen font or require LVGL. Screen strings are sanitized before display to drop emoji, control characters, variation selectors, private-use glyphs, and invalid UTF-8 bytes that the font cannot render.
...
11. **Board/project config**: the package includes `partitions.csv` and a postinstall helper that sets a 16MB custom partition, OPI PSRAM, QIO flash mode, and 921600 upload speed for ESP32-S3 projects when installed in Aily. The audio runtime vendors ArduinoJson and WebSockets into `src.7z` and includes them through `huoshan_ai_ArduinoJson.h` / `huoshan_ai_WebSocketsClient.h`, so local-library imports do not depend on transitive dependency expansion. The helper still ensures optional TFT_eSPI and LVGL dependencies for screen projects, because Aily preprocessing copies only direct project dependencies into `.temp/libraries`. It enables `LV_FONT_FMT_TXT_LARGE` in the generated LVGL config for the bundled screen font and removes stale project-level screen/font macros from older `huoshan-ai-voice` builds before the current screen blocks add their active macros.
